...
def data_view(model: DataViewModel) -> None:
    data = model.data
    ...
    if tex is not None:
        ...
        changed, new_index = imgui.slider_int("##frame_slider", model.frame_index, 0, n - 1)
        if changed:
    ...
    changed, new_index = imgui.input_int("##frame_input", model.frame_index, step=1)
    if changed:
        model.frame_index = max(0, min(n - 1, new_index))

    ts_ns = data.cam_timestamps_ns[model.frame_index]
